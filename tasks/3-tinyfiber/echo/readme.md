...
Действительно, при запуске каждой асинхронной операции нужно
1) Останавливать текущий файбер и планировать его возобновление
2) Прокидывать асинхронный результат

Попробуйте инкапсулировать эту логику внутри класса `Awaiter` из `awaiter.hpp`.

### Awaiter и Future

Две задачи, которые будет решать ваш `Awaiter` – это все же разные задачи, за которые в реальных реализациях будут отвечать разные сущности: `awaiter` и `future`.

- Результат асинхронной операции представляется `Future<T>`
- Логика по возобновлению / восстановлению 
