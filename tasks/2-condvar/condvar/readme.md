...
Изучите:
* [Документацию](https://en.cppreference.com/w/cpp/thread/condition_variable) для `std::condition_variable`
...
В промышленном коде кондвары не следует использовать напрямую. Это низкоуровневый примитив, с помощью которого строятся примитивы для конкретных сценариев синхронизации: барьеры, латчи, каналы, фьючи и т.п. 
