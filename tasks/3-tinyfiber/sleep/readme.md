...
Логика по управлению очередью спящих файберов и связанные структуры данных должны быть инкапсулированы в отдельном классе `SleepQueue`. Тщательно продумайте интерфейс такой очереди. Рекомендуем начать с изменения цикла планировщика: _сначала_ подобрать удобный интерфейс, и только _потом_ писать реализацию.

Идеи вы можете почерпнуть из решения задачи [Sleep-Asio](/tasks/3-tinyfiber/sleep-asio).

## Timer Wheels

Самый эффективный способ строить очередь таймеров – _timer wheels_: 
...
Вы можете изменять содержимое файлов `scheduler.{hpp,cpp}`, `sleep_queue.{hpp,cpp}` и `fiber.{hpp,cpp}`.
