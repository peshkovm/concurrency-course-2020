...
Не делайте публичных конструкторов у `Socket`. Сокеты конструируются
- у клиентов с помощью статических конструкторов `ConnectTo` / `ConnectToLocal`
- в сервере в вызове `acceptor.Accept()`

Метод `ListenAvailablePort` у `Acceptor` реализуйте через `Listen(0)`.

Заголовочный файл `socket.hpp` – публичное API файберов, в нем не должно быть зависимостей от планировщика и т.п.
