...
Если задача _A_ кладет задачу _B_ в пул, и при этом _B_ является логическим _продолжением_ _A_, то вместо `Submit` стоит использовать `SubmitContinuation`. В этом случае задача _B_ попадет в _локальную_ очередь текущего потока-воркера _T_, синхронизация при обращении к глобальной очереди пула не потребуется.
