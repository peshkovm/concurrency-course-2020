...
Токен потребуется для создания merge request-ов из консольного клиента.
...
Команда `tpcc attach` клонирует этот приватный репозиторий локально в соседнюю с репозиторием курса директорию. 

Для каждой задачи в этом репозитории будет создаваться отдельная ветка (например, `0-intro/deadlock`).

Команда `tpcc commit` коммитит файлы теущей задачи (см. `submit_files` в `task.json`) _локально_ в ваш репозиторий решений.
...
3) На задаче в репозитории курса запускаются `tpcc validate`, `tpcc test`, т.е. ровно то же команды, что вы запускаете локально.
