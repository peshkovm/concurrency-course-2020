...
`SubmitContinuation` – это подсказка планировщику, что задачу _B_ можно запланировать локально в тот же поток-воркер, не обращаясь к глобальной очереди пула не потребуется.
