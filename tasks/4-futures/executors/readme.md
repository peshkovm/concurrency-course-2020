...
```cpp
void DoWhile(Function<bool> body, IExecutorPtr strand) {
 strand->Execute([body = std::move(body), strand]() {
   bool cond = body();
   if (cond) {
     DoWhile(std::move(body), strand);
   }
 }
}
```

В этом примере strand выстраивает в цепочку итерации цикла. 
