...
Если задача _A_ кладет задачу _B_ в пул, и при этом _B_ является _продолжением_ (_continuation_) _A_, то вместо `Submit` стоит использовать `SubmitContinuation`. Это подсказка планировщику, что задачу _B_ можно запланировать локально в тот же поток-воркер, без обращения к глобальной очереди пула не потребуется.

Для остановки пула потоков нужно вызвать метод `Join`. После этого вызова потоки-воркеры внутри пула будут работать до тех пор, пока его очередь задач не опустеет. Вызов `Join` вернет управление когда все воркеры завершатся.
