...
Главное содержание задачи – научиться клеить точки старта и завершения асинхронной операции с помощью переключения контекста. В качестве повода для этого выбран `SleepFor`.
