...
// В потоке B:
// "Распаковываем" фьючу, блокируемся до получения результата
...
С помощью фьюч легко реализовать поверх экзекуторов функциональность возврата значения из задачи:
...
* [RPC](https://capnproto.org/cxxrpc.html#promises) к внешнему сервису
* Чтение с диска
...
Продолжение "поглощает" будущий результат, после вызова `Then` обращаться к исходной фьюче нельзя.
...
Приведем простой пример использования комбинаторов для декорирования фьючи: 
