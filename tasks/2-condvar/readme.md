...
- Выбирайте имена для условных переменных так, чтобы они отражали предикат, выполнения которого ждут потоки. Например, условную переменную, на которой метод `Receive` в блокирующей очереди ждет появления элемента, разумно назвать `not_empty_` или `has_items_` или аналогичным образом.
