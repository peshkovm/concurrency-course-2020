...
Вам _уже_ дан готовый многопоточный планировщик, написанный с помощью библиотеки _Asio_.

## Планировщик (пул потоков)

Планировщик ничего не знает про корутины, он исполняет абстрактные _задачи_ – `std::function<void()>`.
...
Задачу в планировщик можно добавить не только снаружи, но и прямо из исполняемой задачи:
...
// Запланировать выполнение внешней задачи
scheduler.Submit(deeper); 
```

Для остановки планировщика нужно вызвать метод `Shutdown`. После этого вызова потоки планировщика будут работать до тех пор, пока его очередь задач не опустеет.
