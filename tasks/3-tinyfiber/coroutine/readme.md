...
scheduler.Run([]() { /* Your task code */ });
```

Вызов `Run` исполняется до тех пор, пока в планировщике остаются невыполненные задачи.

Обратите внимание, планировщик ничего не знает про корутины, он исполняет абстрактные _задачи_ – `std::function<void()>`.
