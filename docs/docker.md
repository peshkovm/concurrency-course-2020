# Установка докера для **tpcc-course**
...
Установить и настроить зависимости можно так: 
```
...
## В случае проблем:
- Если что-то сломалось - сохраните незакоммиченный прогресс где-нибудь в безопасном месте.
- Почитайте логи контейнера:
```
docker logs tpcc-image
```
- Попытайтесь понять, в чем проблема (используя google/man/etc)
- Попробуйте заново провести шаг установки (важно: сохраните незакоммиченный прогресс)
- Если это не помогает, напишите в саппорт чат курса

## Полезные заметки:
Пользователь `user` сделан для интеграции с CLion, не логиньтесь в него специально в ручном режиме.
У этого пользователя может не быть прав на изменение/использование `/tpcc/` и он не предназначен для этого по смысле.
...
- Чтобы использовать отладчик в тестах/коде измените следующие строки в `library/twist/twist/CMakeLists.txt`:
  1. `FORK_TESTS` с 1 на 0
  2. `NO_TEST_TIME_LIMIT` с 0 на 1
